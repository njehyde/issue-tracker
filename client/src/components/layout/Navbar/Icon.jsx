import React from 'react';
import styled from 'styled-components';

const StyledSVG = styled.svg`
  fill: #ffffff;
  stroke: #ffffff;
  transition: all 0.2s ease 0s;

  path {
    transition: inherit;
  }

  &:hover {
    fill: rgba(0, 136, 169, 1);
    stroke: rgba(0, 136, 169, 1);
  }
`;

const StyledPath1 = styled.path`
  fill-rule: evenodd;
  clip-rule: evenodd;
`;

const StyledPath2 = styled.path`
  fill-rule: evenodd;
  clip-rule: evenodd;
  fill: none;
  stroke: inherit;
  stroke-width: 12;
  stroke-linecap: round;
  stroke-linejoin: round;
  stroke-miterlimit: 3;
`;

const Icon = () => (
  <StyledSVG viewBox="0 0 229 229">
    <StyledPath1
      className="st0"
      d="M179.6,23.6c0.4,16-2.7,28.9-9.3,38.5c-4.5,6.5-10.5,11.6-18.1,15.3c-2.2,1.1-4.4,2-6.7,2.8
		c-6.4,2.2-13.5,3.5-21.2,4.1l0.2-6.9c0.2-0.9,0.3-2.2,0.4-4c0.5-4,1.2-7.7,2.2-11.3C133.8,38.6,151.3,25.7,179.6,23.6z"
    />
    <StyledPath2
      className="st1"
      d="M51.7,42.3l67.1,56.4H204c-0.2,34.7-8.9,60.8-26.1,78.3c-17,17.5-37.6,26.3-61.8,26.3h-3.3
		c-24.2-0.1-44.8-8.8-61.9-26.3c-17.1-17.6-25.8-43.7-26-78.3h28.2L27,67.1c-1.2-1.2-1.9-2.7-1.9-4.3c0-0.2,0-0.4,0.1-0.6
		c0.1-1.4,0.7-2.7,1.8-3.8l16-16.1c1.2-1.2,2.6-1.7,4.2-1.7C49,40.5,50.5,41.1,51.7,42.3z M151.5,126.3l-32.8-27.6H53.2l56.9,68.9
		c5.8,5.8,12.7,8.6,20.7,8.5c7.7,0.1,14.3-2.5,19.9-7.9l0.8-0.7c2.7-2.6,4.7-5.6,6.2-8.9c1.5-3.6,2.3-7.5,2.3-11.8
		C160,138.7,157.1,131.9,151.5,126.3z"
    />
  </StyledSVG>
);

export default Icon;
